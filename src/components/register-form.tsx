...
export default function RegisterForm({ t }: { t: Dict }) {
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-ink-950 px-4">
      <div className="w-full max-w-sm rounded-3xl border border-white/10 bg-white/[0.03] p-8">
        ...
        <h1 className="mt-6 text-2xl font-extrabold">{t.authPage.createTitle}</h1>
        <p className="mt-1 text-sm text-slate-400">
          {t.authPage.createDesc}
        </p>

        <form action={action} className="mt-6 space-y-4">
          <div>
            <label htmlFor="name" className="mb-1.5 block text-sm font-medium">
              {t.authPage.nameLabel}
            </label>
            <input
              id="name"
              name="name"
          ...
          >
            {pending ? t.authPage.creating : t.common.createAccount}
          </button>
        </form>
        ...
        <GoogleButton label={t.authPage.googleSignup} />
      </div>
    </div>
  );
}
