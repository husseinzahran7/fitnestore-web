...
- [ ] Or dashboard path: `vercel.com` → Add New → Import `GYMERS` repo → env vars `NEXT_PUBLIC_SUPABASE_URL` + `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` (values in `.env.local`, see `.env.example`) → Deploy.
...
## 3. Appointments — DONE (live)

- [x] Appointments table live, coach plan-a-session form, board with Done/Cancel, .ics download.
- Note: old `fitnestore-hub` proposal retired. App name is GYMERS.
